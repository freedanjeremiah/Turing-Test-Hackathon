...
# Dashboard onboarding: faucet for testnet USDC (operator: replace with the canonical Mantle faucet URL when available)
...
```bash
cd /Users/freedan/Documents/PersonalProjects/pantheon/apps/dashboard
pnpm dev
```
...
2. `pnpm --filter @pantheon/contracts hardhat test` is still green at 24 passing (Phase 3 touches no contracts).
3. `pnpm --filter @pantheon/allocator test` is still green at 9 passing.
4. `grep -rn "TracesFeed" apps/dashboard/src` returns zero matches.
5. `pnpm dev` from `apps/dashboard/` renders the new layout with no console errors. The onboarding strip walks through Connect → Add Mantle Sepolia testnet → Get USDC → Deposit. Each step's button works.
