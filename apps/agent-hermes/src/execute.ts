/**
 * Execute a Hermes trade: open a real perp position on the MantleOraclePerp contract
...
export async function executeHermesTrade(
  ...
  if (!ENABLE_REAL_TRADES) {
    console.log(`[hermes] trade skipped (ENABLE_REAL_TRADES=false): would open ${allocatedUsd} USDC perp for ${proposal.tradeIdea}`);
    return { ok: false, reason: "real_trades_disabled" };
  }

  console.log(`[hermes] opening real Mantle perp with $${allocatedUsd} collateral`);
